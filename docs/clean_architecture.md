...
クリーンアーキテクチャで実装されたアプリケーションはプロジェクトによって名称が異なる
また、場合によってはレイヤーを減らして工数削減を狙ったりすることもある。
レイヤーが細分化されていることでファイル数が多くなり、管理が手間になることもあるが、共通認識の上での実装ができるので知識があるエンジニアがプロジェクトに入った際キャッチアップをの工数を削減できることが多い。
